...
HXLINE( 150)
HXLINE( 152)		HX_VARI(  ::lime::graphics::Image,sourceImage) = sourceBitmapData->image;
HXLINE( 153)		HX_VARI(  ::lime::graphics::Image,image) = destBitmapData->image;
HXLINE( 160)		HX_VARI(  ::lime::utils::ArrayBufferView,sourceData) = sourceImage->get_data();
HXLINE( 161)		HX_VARI(  ::lime::utils::ArrayBufferView,destData) = image->get_data();
HXLINE( 163)		HX_VARI( int,offsetX) = ::Std_obj::_hx_int((destPoint->x - sourceRect->x));
HXLINE( 164)		HX_VARI( int,offsetY) = ::Std_obj::_hx_int((destPoint->y - sourceRect->y));
HXLINE( 165)		HX_VARI( int,sourceStride) = (sourceBitmapData->width * 4);
HXLINE( 166)		HX_VARI( int,destStride) = (destBitmapData->width * 4);
HXLINE( 168)		HX_VARI( int,sourceFormat) = sourceImage->buffer->format;
HXLINE( 169)		HX_VARI( int,destFormat) = image->buffer->format;
HXLINE( 170)		HX_VARI( bool,sourcePremultiplied) = sourceImage->buffer->premultiplied;
HXLINE( 171)		HX_VARI( bool,destPremultiplied) = image->buffer->premultiplied;
HXLINE( 173)		HX_VAR( int,sourcePixel);
HXDLIN( 173)		HX_VARI( int,destPixel) = 0;
HXLINE( 174)		HX_VAR( int,sourceOffset);
HXDLIN( 174)		HX_VAR( int,destOffset);
HXLINE( 176)		{
...
HXLINE( 176)				_g = (_g + 1);
HXDLIN( 176)				HX_VARI( int,row) = (_g - 1);
HXLINE( 178)				{
...
HXDLIN( 178)						HX_VARI( int,column) = (_g1 - 1);
